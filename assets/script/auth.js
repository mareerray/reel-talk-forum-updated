document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateNavMenu() {
    ...
    if (sessionStorage.getItem('sessionToken')) {
      ...
      navMenu.innerHTML = `
        <li><span class="user-display">🍿 ${userNickname}!</span></li>
  ...
  function fetchUserProfile() {
    fetch('/api/user/profile')
      ...
      .then(data => {
        ...
        if (profileNickname) profileNickname.textContent = 'Nickname: ' + data.nickname;
        if (profileFirstname) profileFirstname.textContent = 'First Name: ' + data.first_name;
        if (profileLastname) profileLastname.textContent = 'Last Name: ' + data.last_name;
        if (profileAge) profileAge.textContent = 'Age: ' + data.age;
        if (profileGender) profileGender.textContent = 'Gender: ' + data.gender;
        if (profileEmail) profileEmail.textContent = 'Email: ' + data.email;
        if (pageTitle) pageTitle.textContent = 'Welcome, ' + data.nickname + '!';
      })
      .catch(error => {
  ...
  function handleLogout(event) {
    event.preventDefault();
    if (typeof socket !== 'undefined' && socket) socket.close();
  ...
  function displayError(field, message) {
    ...
    if (!errorElement) {
      ...
      if (inputField) {
        ...
        } else if (field === 'general' && signUpForm) {
          signUpForm.prepend(errorElement);
        } else if (field === 'login-general' && logInForm) {
  ...
  if (signUpForm) {
    signUpForm.addEventListener('submit', function(e) {
        e.preventDefault();
        clearErrors();
       // keep validation + fetch register code here
  ...
  if (logInForm) {
    logInForm.addEventListener('submit', function(e) {
      e.preventDefault();
      clearErrors();
        const selectedOption = document.querySelector('input[name="login-option"]:checked');
        if (!selectedOption) {
        displayError('login-general', 'Please choose nickname or email.');
        return;
        }
        const loginOption = selectedOption.value;
...
//     }
//   showSignIn();
//   updateNavMenu();// Function to redirect to homepage
// function redirectToHomepage() {
//     // Hide the auth view
